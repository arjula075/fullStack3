...
app.get('/info', (request, response) => {
		...
		.find({})
			.then(result => {
					...
					let numberOfNames = 0
					if (obj) {
						numberOfNames = obj.length
					}
					let resultText = `<h1>Info!</h1><p>luettelossa nimiä ${numberOfNames}</p><p>${new Date()}</p>`
...
app.put('/api/persons/:id', (request, response) => {
	...
	updatedPerson.id = parseInt(updatedPerson.id, 16)
...
app.post('/api/persons', (request, response) => {
  ...
  if (note) {
	  ...
	  // this will be commented out for sake of db integrity (nah, testing purposes)
		  ...
		  Person
			...
			.then(result => {
			 ...
			 .catch(error => {
				...
				return response.status(404).json({error: 'MALFORMED request'})
			})
		}
  }
  else {
